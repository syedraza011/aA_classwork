...
var canConstruct = tryRequire("../problems/5_can_construct.js");
...
describe('canConstruct()', function () {
  it('should return true if ransom note can be constructed from magazine', function () {
    assert.equal(canConstruct("a", "aa"), true);
    assert.equal(canConstruct("aabbc", "dbaccab"), true);
    assert.equal(canConstruct("ababc", "dbaccab"), true);
  });

  it('should return false if note can not be formed from magazine', function () {
    assert.strictEqual(canConstruct("ab", "aa"), false);
    assert.strictEqual(canConstruct("aabbc", "aaaabbbdef"), false);
  });
});
